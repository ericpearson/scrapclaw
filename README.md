...
curl -X POST http://localhost:8192/v1 \
  ...
  -d '{"url":"https://example.com","maxTimeout":60000,"wait":0,"responseMode":"html","maxResponseBytes":50000}'
```
...
If `maxResponseBytes` is set, Scrapclaw truncates `solution.response` to that UTF-8 byte limit and sets `solution.truncated` to `true`.
...
That same image is the intended runtime for the GitHub `v0.0.6` release.
...
To publish a container release from this repo, push a semver tag such as `v0.0.6`.
